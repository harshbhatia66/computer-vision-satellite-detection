# This file trains a densenet based mode
...
# Dense network object
class DenseNet(nn.Module):
    def __init__(self):
        super(DenseNet, self).__init__()
        self.densenet = densenet121(weights=DenseNet121_Weights.DEFAULT)
        self.densenet.features.conv0 = nn.Conv2d(1, 64, kernel_size=13, stride=2, padding=3, bias=False)
        # features = 1000
...
def train_then_validate(kernel_size=13, stride=2, padding=3, bias=False, epochs=10):
...
def train_then_test(kernel_size=13, stride=2, padding=3, bias=False, epochs=10):
	net = DenseNet().to(device)
...
test, net = train_then_test(kernel_size=13, stride=2, padding=13, bias=False, epochs=5)